...
void WorldSession::HandleCharFactionOrRaceChange(WorldPacket& recv_data)
{
    ...
    CharacterDatabase.PExecute("DELETE FROM character_declinedname WHERE guid ='%u'", GUID_LOPART(guid));
       ...
       if(recv_data.GetOpcode() == CMSG_CHAR_FACTION_CHANGE)
       {
               ...
               // Eliminamos idiomas, MaNGOS los restaura automaticamente
               CharacterDatabase.PExecute("DELETE FROM `character_skills` WHERE `skill` IN (109, 115, 315, 617, 137) AND `guid`='%u'",GUID_LOPART(guid));
               // Editamos Rutas de vuelo en función del nivel y desmontamos si va en taxi
               ...
               // Elimina registro de quests completadas de la facción antigua(DXN)
               if(team == BG_TEAM_ALLIANCE)
                       CharacterDatabase.PExecute("DELETE FROM character_queststatus WHERE guid= '%u' AND quest IN (SELECT entry FROM mangos.quest_template AS quest WHERE RequiredRaces IN (1,4,8,64,1024,1101))",GUID_LOPART(guid));
               else
                       CharacterDatabase.PExecute("DELETE FROM character_queststatus WHERE guid= '%u' AND quest IN (SELECT entry FROM mangos.quest_template AS quest WHERE RequiredRaces IN (2,16,32,128,512,690))",GUID_LOPART(guid));
               // Reset guild
